...
Esoteric Software's Spine runtime for PixiJS v8, vendored into the framework so Caper can pin it against the framework’s exact `pixi.js` (8.21.0 today), plus a dark-tint batcher Pixi does not ship. Opt-in via `config.useSpine`.
