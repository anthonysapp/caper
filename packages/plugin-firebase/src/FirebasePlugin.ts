import { IApplication, isDev, Logger, Plugin } from '@caperjs/core';
...
interface IFirebasePluginOptions extends FirebaseOptions {
  debug?: boolean;
}
 ...
 */
export class FirebasePlugin extends Plugin<IFirebasePluginOptions> implements IFirebasePlugin {
  private _options: IFirebasePluginOptions;
  ...
  private hello() {
    const hello = `%c Caper Firebase Storage Adapter v${version} | %cFirebase v${firebaseVersion}`;
    console.log(
      hello,
   ...
   * @returns {void}
   */
  public initialize(options: Partial<IFirebasePluginOptions>, _app: IApplication): void {
    const defaultConfig: IFirebasePluginOptions = {
      debug: isDev,
