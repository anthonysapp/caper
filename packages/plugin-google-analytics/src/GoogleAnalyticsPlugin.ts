import type { IApplication, IPlugin } from '@caperjs/core';
import { isDev, Logger, omitKeys, Plugin } from '@caperjs/core';
  ...
  implements IGoogleAnalyticsPlugin<E>
{
  ...
  async initialize(options: Partial<GoogleAnalyticsPluginOptions>, _app: IApplication) {
    ...
    if (!this._options.trackingId) {
      Logger.warn(
        '[Caper Google Analytics Plugin] Google Analytics tracking ID is not set, please set `VITE_GOOGLE_ANALYTICS_TRACKING_ID` or `GOOGLE_ANALYTICS_TRACKING_ID` environment variable.',
      );
      return;
    }
  }
  ...
  private hello() {
    const hello = `%c Caper Google Analytics Plugin v${version}`;
