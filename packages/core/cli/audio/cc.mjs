...
async function readDurations(dir) {
  console.log('readDurations', dir);
  const files = [];
  const outputDir = path.join(dir, '');
  if (!fs.existsSync(outputDir)) {
    return Promise.reject(`Output directory not found at "${outputDir}". Please run \`caper audio compress\` first.`);
  }
  walkDir(outputDir, file => {
...
export async function generateCaptions(dir, outputDir = dir) {
  return readDurations(outputDir).then(() => readCaptions(dir)).then(() => writeCaptions(outputDir));
}
