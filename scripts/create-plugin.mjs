...
const copyAndRenameFiles = (srcDir, destDir, pluginName) => {
	...
	files.forEach(file => {
		...
		if (file === 'Plugin.template.ts') {
			destFileName = `${toCamelCase(pluginName, true)}Plugin.ts`;
		}
...
rl.question('Enter plugin name: ', async (pluginName) => {
	const pluginPath = path.join(cwd, `./packages/plugin-${pluginName}`);
