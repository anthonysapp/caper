...
- Run `pnpm framework:build`, `pnpm kitchen-sink:build`, and `pnpm --filter @caperjs/core test` before opening the PR; all three should be green.
...
- If the change touches the framework's public surface (anything exported from `@caperjs/core`), call it out in the PR description so I can think about the migration story.
...
- Adding a new third-party dependency to `@caperjs/core`. The framework's dep set is deliberately small.
